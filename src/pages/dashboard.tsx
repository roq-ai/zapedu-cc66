...
import { getInstances } from 'apiSdk/instances';
import { InstanceInterface } from 'interfaces/instance';

import { AgentListPage as ShowCaseList } from 'pages/agents';

function DashboardPage() {
  const { session } = useSession();

  const agentStat = useSWR<PaginatedInterface<AgentInterface>>(
    () => `/agents`,
    () => getAgents({ limit: 1, offset: 0 }),
  );

  const instanceStat = useSWR<PaginatedInterface<InstanceInterface>>(
    () => `/instances`,
    () => getInstances({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Agents',
      result: agentStat?.data?.totalCount,
      isLoading: agentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Instances',
      result: instanceStat?.data?.totalCount,
      isLoading: instanceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
