...
import { ZapEduInterface } from 'interfaces/zap-edu';
import { getZapEdus } from 'apiSdk/zap-edus';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['administrator', 'user', 'instance-manager', 'chat-tester', 'agent'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ZapEduInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/zap-edus` : null),
    () => getZapEdus({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Zap Edus',
      path: isTenantUser ? `/zap-edus/view/${data?.[0]?.id}` : '/zap-edus',
      entity: 'zap_edu',
      service: AccessServiceEnum.PROJECT,
      icon: FiBox,
    },
    { name: 'Agents', path: '/agents', entity: 'agent', service: AccessServiceEnum.PROJECT, icon: FiBox },
    { name: 'Instances', path: '/instances', entity: 'instance', service: AccessServiceEnum.PROJECT, icon: FiBox },
